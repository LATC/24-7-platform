...
public class CommandLine {
	...
	public CommandLine() {
		...
		parser.accepts(HADOOP_PATH, "[REQUIRED] The path of hadoop instalation directory").withRequiredArg()
				.ofType(String.class);
		parser.accepts(HADOOP_USER, "[OPTIONAL] HDFS (hadoop distributed filesystem) user for running at hbase")
		...
		parser.accepts(LINKS_FILE,
				"[OPTIONAL] The name of link file for storing the triples of links generation, default : links.nt")
				.withRequiredArg().ofType(String.class);
		parser.accepts(SPEC_FILE, "[OPTIONAL] The name of SILK specification file , default : spec.xml")
				.withRequiredArg().ofType(String.class);
		parser.accepts(VOID_FILE, "[OPTIONAL] The name of void file , default : void.ttl").withRequiredArg()
				.ofType(String.class);
		parser.accepts(RESULT_LOCAL_DIR, "[OPTIONAL] The local path result , default : results").withRequiredArg()
				.ofType(String.class);
		parser.accepts(API_KEY_CONSOLE, "[REQUIRED] The api key for posting report to onsole ").withRequiredArg()
				.ofType(String.class);
		parser.accepts(API_KEY_MDS, "[REQUIRED] The api key for posting report to onsole ").withRequiredArg()
				.ofType(String.class);
	...
	public void run(final String[] args) throws Exception {
		try {
			...
			if (!options.has(HADOOP_PATH) || !options.has(LATC_CONSOLE_HOST) || !options.has(RESULTS_HOST)
					|| !options.has(API_KEY_CONSOLE) || !options.has(API_KEY_MDS) || !options.has(MDS_HOST)) {
				System.out
						.println("Error parameter hadoop path, URL of console host, api key and URL of links generation are required");
				parser.printHelpOn(System.out);
				System.exit(0);
			}
	...
	/**
	 * @param args
	 *            Option Description ------ ----------- -c [OPTIONAL] The path
	 *            of configuration file --hadoop-path [REQUIRED] The path of
	 *            hadoop instalation directory --hadoop-user [OPTIONAL]
	 *            [OPTIONAL] HDFS (hadoop distributed filesystem) user for
	 *            running at hbase --help print usage information
	 *            --latc-console-host [REQUIRED] The URL of console host
	 *            --links-file [OPTIONAL] The name of link file for storing the
	 *            triples of links generation, default : links.nt
	 *            --result-local-dir [OPTIONAL] The local path result , default
	 *            : results --results-host [REQUIRED] The URL of links
	 *            generation --spec-file [OPTIONAL] The name of SILK
	 *            specification file , default : spec. xml --void-file
	 *            [OPTIONAL] The name of void file , default : void.ttl
	 * 
	 * @throws Exception
	 */
